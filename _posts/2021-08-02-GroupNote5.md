...
- (5):构建矢量空间$V$中的所有函数${T_j\rvert\mathbf{x}_j}\psi_{\mathbf{k}_1,i},i=1,q;i=1,t$,这里的${T_j\rvert\mathbf{x}_j}$是空间群根据小群$\mathbf{G}^{\mathbf{k}_1}$的左陪集表示,此时矢量空间$V$对于空间群$G$而言是不可约的,并且表示$\Gamma$的基函数有$d$个${T_j\rvert\mathbf{x}_j}\psi_{\mathbf{k}_1,i}(\mathbf{r})$函数,这里$dqt=ht/b$.
