...
$$(r_1t_\alpha)$$的逆为$$(r_1t_\alpha^{-1})$$.给定一个$r_\alpha$一定存在一个唯一的陪集表示$$r_{\alpha\hat{\alpha}}=r_{\hat{\alpha}\alpha}=r_1$$,$$(r_\alpha t_1)$$的逆是$$(r_\alpha t_{\alpha\alpha}^{-1})$$,还有下面的一些关系

$$t_{\hat{\alpha}\alpha}=[t_{\alpha\hat{\alpha}}]_\alpha$$

$$[[t_a]_\alpha]_\hat{\alpha}=t_{\alpha\hat{\alpha}}^{-1}t_at_{\alpha\hat{\alpha}}$$
